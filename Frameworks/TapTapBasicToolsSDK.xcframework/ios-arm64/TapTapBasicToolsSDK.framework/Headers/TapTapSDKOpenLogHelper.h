...
///   - actionType: 埋点事件类型
///   - properties: 埋点参数
+ (void)reportBusinessLog:(TapTapSDKProjectType)project  actionType:(OpenLogEventActionType) actionType properties:(nullable NSDictionary *)properties;

/// 业务埋点 （上报 tapsdk表）
/// - Parameters:
///   - projcet:模块名称
///   - eventKey: 事件 KEY, 一次接口调用后可能内部会有多个事件并行执行，例如成就，所以使用该字段来区分不同事件
///   - actionType: 埋点事件类型
///   - properties: 埋点参数
+ (void)reportBusinessLog:(TapTapSDKProjectType)project  eventKey:(nullable NSString *) eventKey actionType:(OpenLogEventActionType) actionType properties:(nullable NSDictionary *)properties;
