 ...
 * 成功返回 0，失败返回 -1
 */
TAPSDK_EXPORT_API int TapSdkBifrostInitialize(const char* jsonCfg);

/**
 * 程序退出前调用一次，关闭连接、释放资源。非线程安全，并发调用可能导致崩溃。
 */
TAPSDK_EXPORT_API void TapSdkBifrostFinalize();
 ...
 */
TAPSDK_EXPORT_API void TapSdkBifrostSetLogLevel(int logLevel, int logToConsole);

/**
 * 代码版本，如：1.2.5
 */
TAPSDK_EXPORT_API const char* TapSdkBifrostVersion();

/**
 * git commit 版本，如：98f5d81a0fdcab9a755878b3e825c2cb510e5196
 */
TAPSDK_EXPORT_API const char* TapSdkBifrostGitCommit();
 ...
 */
TAPSDK_EXPORT_API int TapSdkBifrostSubscribeService(const char* jsonCfg, SubscribeCallback subCB, MessageCallback msgCB, RequestCallback reqCB);
 ...
 * @param svc 想要退订的服务
 */
TAPSDK_EXPORT_API void TapSdkBifrostUnsubscribeService(const char* svc);
 ...
 *   - -2 未初始化。尚未调用 TapSdkBifrostInitialize()，或者初始化失败，或者已经调用了 TapSdkBifrostFinalize()
 */
TAPSDK_EXPORT_API int TapSdkBifrostSendRequest(uint64_t reqID, const char* svc, uint32_t svcCmd, const void* data, uint32_t dataLen);
